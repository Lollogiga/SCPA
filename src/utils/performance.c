...
#define CSV_HEADER "Implementation,Format,Matrix,NZ,Repetitions,AvgTime_ms,GFLOPS,Threads,BlockSize,BlocksPerGrid,HasError,MaxAbsError,MaxRelError,AvgAbsError,AvgRelError,ErrorL2\n"
...
void csv_logger_write(FILE* fp, const PerformanceResult* result) {
    ...
    if (result->has_error) {
        fprintf(
            ...
            // fp, "%s,%s,%s,%d,%d,%.6f,%.2f,%d,%d,%d,%d,%.6f,%.6f,%.6f,%.6f,%.6f\n",
            fp, "%s,%s,%s,%d,%d,%f,%f,%d,%d,%d,%d,%f,%f,%f,%f,%f\n",
            ...
            result->blockSize,              // Numero di thread per blocco in CUDA
            result->blocksPerGrid,          // Numero di blocchi per griglia in CUDA
        ...
        fprintf(
            // TODO capire la granularità che ci interessa avere sul file csv
            // fp, "%s,%s,%s,%d,%d,%.6f,%.2f,%d,%d,%d\n",
            fp, "%s,%s,%s,%d,%d,%f,%f,%d,%d,%d\n",
            result->implementation,         // Tipo di implementazione ("OpenMP" o "CUDA")
            result->format,                 // Formato della matrice ("CSR", "HLL", etc.)
