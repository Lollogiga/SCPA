...
int csrProduct_OpenMP(CSRMatrix *csrMatrix, MatVal *vector, const int num_threads, const ResultVector *serial_res, PerformanceResult *performance) {
    ...
    // Csr solution 1
    BEGIN_BENCHMARK(performance, "csr_openmpProduct_sol1")
    start = omp_get_wtime();
    ...
    free_ResultVector(res);
    END_BENCHMARK(performance)
    printf("csr_openmp1: GFLOPS = %lf\n", computeFlops(NZ, performance->avg_time_ms));

    // Csr solution 2
    BEGIN_BENCHMARK(performance, "csr_openmpProduct_sol2")
    start = omp_get_wtime();
    ...
    // Csr solution 3
    BEGIN_BENCHMARK(performance, "csr_openmpProduct_sol3")
    start = omp_get_wtime();
    ...
    // Csr solution 4
    BEGIN_BENCHMARK(performance, "csr_openmpProduct_sol4")
    start = omp_get_wtime();
    ...
    BEGIN_BENCHMARK(performance, "csr_openmpProduct_sol5")
    start = omp_get_wtime();
...
int hllProduct_OpenMP(HLLMatrix *hllMatrix, MatVal *vector, int num_threads, ResultVector *serial_res, PerformanceResult *performance) {
    ...
    // OpenMP solution 1
    BEGIN_BENCHMARK(performance, "hll_openmpProduct_sol1")
    start = omp_get_wtime();
    ...
    // OpenMP solution 2
    BEGIN_BENCHMARK(performance, "hll_openmpProduct_sol2")
    start = omp_get_wtime();
    ...
    BEGIN_BENCHMARK(performance, "hll_openmpProduct_sol3")
    start = omp_get_wtime();
